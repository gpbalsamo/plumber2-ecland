...
MODEL_RE = re.compile(r'ecLand_PLUMBER2_([A-Za-z0-9\-]+)_(\d{4}-\d{4})\.nc')
...
def discover_pairs(flux_dir: Path, model_dir: Path) -> list[tuple[str, str, Path, Path]]:
    ...
    model_map = {}
    for f in sorted(model_dir.glob('ecLand_PLUMBER2_*.nc')):
...
def process_site(site: str, period: str, flux_path: Path, model_path: Path) -> dict[str, Any] | None:
    ...
    record = {
        ...
        'period': period,
        'years': nanround(common_time.size / (48 * 365.25), 2),
        'metrics': {},
        'monthly_clim': {},
        'diurnal': {},
        'trend': {},
...
def main() -> None:
    ...
    metrics_csv = args.out_dir / 'plumber2_benchmark_metrics.csv'
    pd.DataFrame(rows).to_csv(metrics_csv, index=False)
    print(f'Wrote {metrics_csv}')

    data_json = args.out_dir / 'plumber2_benchmark_data.json'
    data_str = json.dumps({
    ...
    if DASHBOARD_TEMPLATE.exists():
        dashboard_html = args.out_dir / 'plumber2_benchmark_dashboard.html'
