...
# Download the PLUMBER2 observed flux (evaluation) dataset v1.0 from its
...
# sites. This script fetches the raw per-site "*_Flux.nc" files (observed
# latent/sensible heat flux, NEE, GPP, etc.) used to evaluate ecLand
# output against tower measurements.
#
# Reference:
...
#
# Companion script: get_plumber2_forcing.sh (meteorological forcing data).

set -euo pipefail

CATALOG_URL="${CATALOG_URL:-https://thredds.nci.org.au/thredds/catalog/ks32/CLEX_Data/PLUMBER2/v1-0/Flux/catalog.xml}"
FILESERVER_BASE="${FILESERVER_BASE:-https://thredds.nci.org.au/thredds/fileServer}"
OUTDIR="${OUTDIR:-${HOME}/Work/plumber2-ecland/flux/PLUMBER2_original}"
PATTERN="${PATTERN:-.*\.nc$}"
